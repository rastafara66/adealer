...
## 17.0.1.11.2 — 2026-09-06 — перший випуск для Odoo 17

Порт із серії 18.0 (та сама версія `1.11.2`, лише інша серія).

### Changed

- **Списковий вигляд зветься `tree`, а не `list`** (`list` з'явився у 18.0):
  вигляди й `view_mode` у діях, зокрема три в Python.
- **Ліниві переклади:** у Odoo 17 `_lt` — це сам клас із
  `odoo.tools.translate`, а фабрики `LazyTranslate(__name__)` (18.0+) немає.
  Імпорт тепер працює на обох серіях, виклик не змінився.
- 🔴 **Три xpath цілились у структуру, якої в Odoo 17 немає:**
  - у канбані авто фото малюється тегом `<img>` із `kanban_image()`, а не
    полем `image_128` (полем воно стало у 18.0);
  - у канбані замовлення немає `<footer>`, а партнер і номер виводяться через
    `t-out`, не полями.
    ⚠️ Один із цих xpath у 17-й **знаходився** — але в блоці оголошення полів
    угорі картки. Тобто логотип не впав би з помилкою, а просто **ніде не
    намалювався**: тиха порожнеча, найгірший клас помилки (конвенції §7).
- 🔴 **Поле з модифікатора мусить бути присутнє у вигляді** — Odoo 17 цього
  вимагає, 18 і 19 терплять його відсутність. `source_sale_order_id` додано
  прихованим; без нього модуль падав прямо при встановленні.

### Перевірено

- Встановлення в **чисту** базу на живому Odoo 17: `installed`, 16 моделей,
  67 меню, 33 правила доступу, 83 вигляди, 59 дій.
- **49 тестів, 0 падінь, 0 помилок.**

## 18.0.1.11.2 — 2026-09-06 — перший випуск для Odoo 18

Порт із серії 19.0 (та сама версія `1.11.2`, лише інша серія). Функціонально
модуль той самий.

### Changed

- `models.Constraint` (з'явився в Odoo 19) замінено на `_sql_constraints` —
  обмеження унікальності у звітах про помилки. Те саме обмеження, оголошене
  інакше.
- 🔴 **xpath у формі замовлення цілився в кнопки, яких в Odoo 18 немає під
  такими іменами.** Кнопки «Надіслати поштою» у 18-й звуться
  `send_by_email_primary` / `send_by_email`, а в 19.0 їх перейменували на
  `quotation_send_primary` / `quotation_send`. Модуль на 18-й падав **прямо
  при встановленні**: `Element <xpath ...> cannot be located in parent view`.

### Перевірено

- Встановлення в **чисту** базу на живому Odoo 18.0-20250626: `installed`,
  16 моделей, 67 меню, 33 правила доступу, 83 вигляди, 59 дій.
- **49 тестів, 0 падінь, 0 помилок** (разом із темою).
